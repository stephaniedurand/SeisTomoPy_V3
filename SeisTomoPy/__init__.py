...
def cross_section_plot(model,para,elat,elon,slat,slon,depth,NSmax,Vmax,earthquakes=True,hotspots=True):
	...
	path = DIR + "/fortran_files/output_files_cross"
	dirs = os.listdir(path)
	os.chdir(DIR + '/fortran_files/output_files_cross')
...
def tomomap_plot(model,para,depth,NSmax,Vmax,lon0=140,plate_boundaries=True,hotspots=True):
    ...
    path = DIR + "/fortran_files/output_files_map"
    dirs = os.listdir(path)
    os.chdir(DIR + '/fortran_files/output_files_map')
    for i in range(len(dirs)):
    	os.remove(dirs[i])
    os.chdir(DIR_OR)
    # os.chdir('../../')
...
def path_plot(model,para,Vmax,elat,elon,slat,slon,EVT,STA,liste):
	...
	auxa.scatter(180.-(STA[:,0]),6371.-(STA[:,1]), s=80,zorder=3,\
                                           color="black", marker="^",\
                                           edgecolor="k",clip_on=False)

	auxa.scatter(180.-(EVT[:,0]),6371.-(EVT[:,1]), s=80,zorder=3,\
                                           color="red", marker="*",\
                                           edgecolor="k",clip_on=False)
	...
	for i in range(len(EVT)):
		for j in range(len(STA)):
			...
			for k in range(nber_phase):
				...
				auxa.plot(180.-path_deg,6371.-path_depth,color="black",LineStyle="-",clip_on=False)
...
def path_plot_fromfile(file,th,r,para,elat,elon,slat,slon,Vmax,EVT,STA,liste):
	...
	shutil.copy2(os.path.abspath(file),DIR + '/fortran_files/input_files/cross_section_path.xyz')
...
def cross_section(model,para,elat,elon,slat,slon,depth,NSmax):
	...
	Z = np.loadtxt(DIR + '/fortran_files/coupes_NEW_' + str(model) + '_' + str(para)  + '.res')
...
def tomomap(model,para,depth,NSmax):
	...
	filename = DIR + '/fortran_files/output_files_map/map_NEW_' + str(model) + '_' + str(para) + '.xyz'
...
def spectrum(model,para,depth,NSmax):
	...
	path = DIR + "/fortran_files/output_files_spectre"
	dirs = os.listdir(path)
	os.chdir(DIR + '/fortran_files/output_files_spectre')
	for i in range(len(dirs)):
	...
	os.chdir(DIR + '/fortran_files/')
	os.system('csh make_map.csh ' +  str(depth) + ' ' + str(model_choice)+ ' ' + str(NSmax)) 
	filename5 = 'output_files_map/map_NEW_'+ model +'_'+ para+'.xyz'
	filename6 = 'input_files/map_NEW_'+ model +'_'+ para+'.xyz'
	shutil.copy2(filename5,filename6)
	os.system('csh make_spectre.csh ' + str(NSmax) + ' ' + str(model_choice) + ' ' + str(para1) + ' ' + str(depth))
	filename4 = 'output_files_spectre/spectre_' +str(model)+ '_'+str(para)+'_'+str(depth)+'km.xy'
...
def spectrum_fromfile_GUI(file,NSmax):
	...
	filename4 = 'output_files_spectre/spectre_fromfile.xy'
	sp = np.loadtxt(filename4)
	os.chdir(DIR_OR)
	# os.chdir('../')
...
def spectrum_fromfile(file,NSmax):
	...
	shutil.copy2(os.path.abspath(file),DIR + '/fortran_files/input_files/map_file.xyz')
	file_new = '../input_files/map_file.xyz'
...
def correlation(model1,model2,depth1,depth2,para1,para2,NSmax):
	...
	path = DIR + "/fortran_files/output_files_corr"
	dirs = os.listdir(path)
	os.chdir(DIR + '/fortran_files/output_files_corr')
	for i in range(len(dirs)):
		os.remove(dirs[i])
	os.chdir(DIR_OR)
	# os.chdir('../../')
	...
	filename5 = 'output_files_map/map_NEW_'+ model1 +'_'+ para1+'.xyz'
	filename6 = 'input_files/map_NEW_'+ model1 +'_'+ para1+'_'+ str(depth1)+'.xyz'
	shutil.copy2(filename5,filename6)
	os.system('csh make_map.csh ' +  str(depth2) + ' ' + str(model_choice2)+ ' ' + str(NSmax) )
	filename5 = 'output_files_map/map_NEW_'+ model2 +'_'+ para2+'.xyz'
	filename6 = 'input_files/map_NEW_'+ model2 +'_'+ para2+'_'+ str(depth2)+'.xyz'
	shutil.copy2(filename5,filename6)
	os.system('csh make_corr.csh ' + str(NSmax) + ' ' + str(depth1) + ' ' + str(model_choice1) + ' ' + str(para11) + ' ' + str(depth2)+ ' ' + str(model_choice2) + ' ' + str(para22))
	filename4 = 'output_files_corr/corr_'+str(model1)+'_'+str(model2)+'.xy'
...
def correlation_fromfile_GUI(model1,depth1,para1,file,NSmax):
	...
	filename4 = 'output_files_corr/corr_fromfile_'+ str(model1) +'.xy'
	corr = np.loadtxt(filename4)
	os.chdir(DIR_OR)
	# os.chdir('../')
...
def correlation_fromfile2_GUI(file1,file2,NSmax):
	...
	filename4 = 'output_files_corr/corr_fromfile.xy'
	corr = np.loadtxt(filename4)
	os.chdir(DIR_OR)
	# os.chdir('../')
...
def correlation_fromfile(model1,depth1,para1,file,NSmax):
	...
	shutil.copy2(os.path.abspath(file),DIR + '/fortran_files/input_files/map_file1.xyz')
	file_new = '../input_files/map_file1.xyz'
...
def correlation_fromfile2(file1,file2,NSmax):
	...
	shutil.copy2(os.path.abspath(file1),DIR + '/fortran_files/input_files/map_file1.xyz')
	file1_new = '../input_files/map_file1.xyz'
	shutil.copy2(os.path.abspath(file2),DIR + '/fortran_files/input_files/map_file2.xyz')
	file2_new = '../input_files/map_file2.xyz'
...
def get_travel_time_list(model1,elat,elon,edepth,slat,slon,PHASES,PATH_INTERPOLATION = False):
    ...
    model2D = np.loadtxt(DIR + '/fortran_files/output_files_cross/' + str(model1) + '_PathPy.sph')
...
def get_travel_time(model1,elat,elon,edepth,slat,slon,PHASES,PATH_INTERPOLATION = False):
    ...
    # load the interpolated PREM model
    PREM = np.loadtxt(DIR + '/Taup_models/prem/prem_interpolated.txt')
    PREM_depth = PREM[:,0] # depth in km
    PREM_VP = PREM[:,1] # km/s
    PREM_VS = PREM[:,2] # km/s
    PREM_RHO = PREM[:,3]
    PREM_r = 6371.0 - PREM_depth # radius in km
    # load the interpolated PREM mantle  model
    PREM_mantle = np.loadtxt(DIR + '/Taup_models/prem/prem_mantle_interpolated.txt')
    PREM_mantle_depth = PREM_mantle[:,0] # depth in km
    PREM_mantle_VP = PREM_mantle[:,1] # km/s
    PREM_mantle_VS = PREM_mantle[:,2] # km/s
    PREM_mantle_RHO = PREM_mantle[:,3]
    PREM_mantle_r = 6371.0 - PREM_mantle_depth # radius in km
    # load the interpolated PREM outercore  model
    PREM_outercore = np.loadtxt(DIR + '/Taup_models/prem/prem_outercore_interpolated.txt')
    PREM_outercore_depth = PREM_outercore[:,0] # depth in km
    PREM_outercore_VP = PREM_outercore[:,1] # km/s
    PREM_outercore_VS = PREM_outercore[:,2] # km/s
    PREM_outercore_RHO = PREM_outercore[:,3]
    PREM_outercore_r = 6371.0 - PREM_outercore_depth # radius in km
    # load the interpolated PREM innercore model
    PREM_innercore = np.loadtxt(DIR + '/Taup_models/prem/prem_innercore_interpolated.txt')
    PREM_innercore_depth = PREM_innercore[:,0] # depth in km
    PREM_innercore_VP = PREM_innercore[:,1] # km/s
    PREM_innercore_VS = PREM_innercore[:,2] # km/s
    PREM_innercore_RHO = PREM_innercore[:,3]
...
################################
    ...
    # load the interpolated PREM model
    REF = np.loadtxt(DIR + '/Taup_models/'  + model_choice1 +'/'+ model_choice1  + '_interpolated.txt')
    REF_depth = REF[:,0] # depth in km
    REF_VP = REF[:,1] # km/s
    REF_VS = REF[:,2] # km/s
    REF_RHO = REF[:,3]
    REF_r = 6371.0 - REF_depth # radius in km
    # load the interpolated REF mantle  model
    REF_mantle = np.loadtxt(DIR + '/Taup_models/'  + model_choice1 +'/'+ model_choice1  + '_mantle_interpolated.txt')
    REF_mantle_depth = REF_mantle[:,0] # depth in km
    REF_mantle_VP = REF_mantle[:,1] # km/s
    REF_mantle_VS = REF_mantle[:,2] # km/s
    REF_mantle_RHO = REF_mantle[:,3]
    REF_mantle_r = 6371.0 - REF_mantle_depth # radius in km
    # load the interpolated REF outercore  model
    REF_outercore = np.loadtxt(DIR + '/Taup_models/'  + model_choice1 +'/'+ model_choice1  + '_outercore_interpolated.txt')
    REF_outercore_depth = REF_outercore[:,0] # depth in km
    REF_outercore_VP = REF_outercore[:,1] # km/s
    REF_outercore_VS = REF_outercore[:,2] # km/s
    REF_outercore_RHO = REF_outercore[:,3]
    REF_outercore_r = 6371.0 - REF_outercore_depth # radius in km
    # load the interpolated REF innercore model
    REF_innercore = np.loadtxt(DIR + '/Taup_models/'  + model_choice1 +'/'+ model_choice1  + '_innercore_interpolated.txt')
    REF_innercore_depth = REF_innercore[:,0] # depth in km
    REF_innercore_VP = REF_innercore[:,1] # km/s
    REF_innercore_VS = REF_innercore[:,2] # km/s
    REF_innercore_RHO = REF_innercore[:,3]
    ...
    return tt2D, dtt2D, ttREF, phase_name[0]
...
def get_travel_time_fromfile(file,elat,elon,edepth,slat,slon,PHASES,PATH_INTERPOLATION = False):
    ...
    shutil.copy2(os.path.abspath(file),DIR + '/fortran_files/output_files_cross/path.xyz')
    file_new = DIR + '/fortran_files/output_files_cross/path.xyz'
