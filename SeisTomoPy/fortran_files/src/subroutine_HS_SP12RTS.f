...
!      Transforme les coefficients d'harmoniques spheriques en coordonnees geographiques
       ...
       real(kind=8) dvvst(3721,579),dvvpt(3721,579)
...
c       open(1,file='../models/S40RTS.sph')
c       cd=1
c       do depth=2890,50,-5
c
c!      Comme dans S40RTS
...
c       Ralpha(cd)=1.d0/Ralpha(cd)
        Rrho=50.d-2
c
c       read(1,'(1681f15.8)') (dvvst(s,cd),s=1,1681) 
c!       write(*,'(1681f15.8)') (dvvst(s,cd),s=1,1681) 
c       cd=cd+1
c       enddo
c       close(1)
       ...
       open(13,file='../output_files_cross/output_cross_SP12RTS.out')
       open(14,file='../output_files_cross/SP12RTS_input_AxiSEM.sph')
       open(15,file='../output_files_cross/SP12RTS_PathPy.sph')
       write(14,*) (width+1)*cd
       do kk=1,width+1
...
c       write(*,*) ll,ind
       ...
       splitff=splitff+1.d0*x(1)*dvvpt(ind,radius)
       ind=ind+1
     ...
     +            +(2.d0)*dvvst(ind,radius)*(dcos(dble(mm)*phi))*x(mm+1)
       splitff=splitff
     +            +(2.d0)*dvvpt(ind,radius)*(dcos(dble(mm)*phi))*x(mm+1)
       ind=ind+1
       splitf=splitf
     +           +(2.d0)*dvvst(ind,radius)*(dsin(dble(mm)*phi))*x(mm+1)
       splitff=splitff
     +           +(2.d0)*dvvpt(ind,radius)*(dsin(dble(mm)*phi))*x(mm+1)
       ind=ind+1
c       write(*,*) ll,ind-1
       enddo
       endif
       enddo
       write(10,*) deg,6371-depth,splitff*100.d0
       write(11,*) deg,6371-depth,splitf*100.d0
       write(12,*) deg,6371-depth,Rrho*splitf*100.d0
       write(13,*) ttheta,pphi,6371-depth,splitff*100.d0,
     + splitf*100.d0,Rrho*splitf*100.d0
       write(14,*) 6371-depth,deg,splitff*100.d0,
     + splitf*100.d0,Rrho*splitf*100.d0
       write(15,*) 6371-depth,kk-1,splitff*100.d0,
     + splitf*100.d0,Rrho*splitf*100.d0
