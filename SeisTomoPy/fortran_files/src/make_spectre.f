...
c       implicit real*8 (a-h,o-z)
       ...
       integer llmax,xxx,xxxx,i,j,ll,l,ind,mm,tomo,para,depth
     ...
     + fichier4(nx,ny)
       ...
       character*15 namemodel,namepara
       character*4 toto,namedepth
...
c
c   read filename
       read(*,*) llmax,tomo,para,depth
       write(namedepth,'(i4)') depth
       if (tomo.eq.1) namemodel = "SEISGLOB2"
       if (tomo.eq.2) namemodel = "S40RTS"
       if (tomo.eq.3) namemodel = "SEMUCBWM1"
       if (tomo.eq.4) namemodel = "S362WMANIM"
       if (tomo.eq.5) namemodel = "SEISGLOB1"
       if (tomo.eq.6) namemodel = "SP12RTS"
       if (tomo.eq.7) namemodel = "SGLOBE"
       if (tomo.eq.8) namemodel = "3D2016"
       if (tomo.eq.9) namemodel = "MYMODEL"
       if (para.eq.1) namepara = "VS"
       if (para.eq.2) namepara = "VP"
       if (para.eq.3) namepara = "RHO"
       filemoi1=trim(adjustl(homedir))//"input_files/map_NEW_"
     + //trim(adjustl(namemodel))//
     + "_"//trim(adjustl(namepara))//".xyz"
       filemoi2=trim(adjustl(homedir))//
     + "output_files_spectre/spectre_"//
     + trim(adjustl(namemodel))//"_"//
     + trim(adjustl(namepara))//"_"//trim(adjustl(namedepth))//
     + "km.xy"
...
c   5. Compute spetcrum 
       call spec(yyd1,llmax,sp,sptotal)
