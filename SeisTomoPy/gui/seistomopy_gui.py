...
################################################################
    ...
    def plot_map(self):
        PC=np.loadtxt(DIR + '/points_chauds.xy')
    ...
    def on_plot_cross_released(self):
        ...
        EQ1=np.loadtxt(DIR + '/../fortran_files/eq_coupe.xy')
        pcval = self.ui.hotspots.checkState()
        PC3=np.loadtxt(DIR + '/../fortran_files/points_chauds_coupe.xy')
        ...
        PL=np.loadtxt(DIR + '/../fortran_files/points_ligne_coupe.xy')
    ...
    def on_saveoutput_cross_released(self):
        ...
        if os.path.isdir(str(selected_directory2) + '/output_crosssection_' +\
         str(lati) + '_' + str(longi) + '_' + str(azi_cross)):
            ...
            shutil.copytree(DIR + '/../fortran_files/output_files_cross', str(selected_directory2) + '/output_crosssection_' +\
         str(lati) + '_' + str(longi) + '_' + str(azi_cross))
...
################################################################
    ...
    def on_saveoutput_map_released(self):
        ...
        if os.path.isdir(str(selected_directory5) + '/output_map_' + \
            ...
            shutil.copytree(DIR + '/../fortran_files/output_files_map', str(selected_directory5) + '/output_map_' + \
             str(dep_map) + 'km')
    ...
    def on_plot_map_released(self):
        ...
        Z2, lat, lon = SeisTomoPy.tomomap(model_map,para,dep_map,NSmax_map)
    ...
    def _plot_hotspots_map(self):
        ...
        if resample2:
            PC2=np.loadtxt(DIR + '/points_chauds.xy')
...
################################################################
    ...
    def on_path_plot_released(self):
        ...
        Z = np.loadtxt(DIR + '/../fortran_files/input_files/cross_section_path.xyz')
        ...
        # #                         transform=self.ax1_cross.transData._b, zorder=2))
        stafile = DIR + '/../fortran_files/input_files/station_file_path.xy'
        evtfile = DIR + '/../fortran_files/input_files/event_file_path.xy'
        if os.path.exists(stafile):
            STA = np.loadtxt(DIR + '/../fortran_files/input_files/station_file_path.xy')
            self.__station = self.auxa.scatter(180.-(STA[:,0]),6371.-(STA[:,1]), s=80,zorder=3,\
                                               color="black", marker="^",\
                                               edgecolor="k",clip_on=False)
            os.remove(DIR + '/../fortran_files/input_files/station_file_path.xy')
        if os.path.exists(evtfile):
            EVT = np.loadtxt(DIR + '/../fortran_files/input_files/event_file_path.xy')
            self.__event = self.auxa.scatter(180.-(EVT[:,0]),6371.-(EVT[:,1]), s=80,zorder=3,\
                                               color="red", marker="*",\
                                               edgecolor="k",clip_on=False)
            os.remove(DIR + '/../fortran_files/input_files/event_file_path.xy')
        os.remove(DIR + '/../fortran_files/input_files/cross_section_path.xyz')
        ...
        if str(self.ui.lineEdit.text()):
            ...
            for i in range(len(EVT)):
                for j in range(len(STA)):
                    arrivals = model.get_ray_paths(source_depth_in_km=EVT[i,1],distance_in_degree=abs(STA[j,0]-EVT[i,0]),phase_list=eval(str(liste_ph)))
                    ...
                    for k in range(nber_phase):
                        ...
                        self.__path = self.auxa.plot(180.-path_deg,6371.-path_depth,linewidth=1.0,color="black",linestyle="-",clip_on=False)
    ...
    def on_loadfile_path_released(self):
        ...
        if self.corr_file3.endswith('.sph'):
            ...
            filename7 = DIR + '/../fortran_files/input_files/cross_section_path.xyz'
    ...
    def on_loadfile_st_released(self):
        ...
        if self.corr_file5.endswith('.xy'):
            ...
            filename7 = DIR + '/../fortran_files/input_files/station_file_path.xy'
            	...
            	if len(arrivals) == 0:
                    ...
                    if arrival.name != PHASES[i_phase]:
                        ...
                        if arrival.name not in p_phases and arrival.name not in s_phases and arrival.name not in added_converted_phases:
                            ...
                            tt, dtt, tref, phase_name = SeisTomoPy.get_travel_time(model_path,ELAT[k],ELON[k],EDEPTH[k],SLAT[k],SLON[k],[PHASES[i_phase]],PATH_INTERPOLATION = False)
                            file_str = 'event: ' + ' LAT: ' + str(ELAT[k]) + ' LON: ' + str(ELON[k]) + ' DEPTH: ' +  str(EDEPTH[k]) + ' station: ' + 'LAT: ' + str(SLAT[k]) + ' LON: ' + str(SLON[k]) + '\n'
                            solution_file.write(file_str)
                            file_str = str(phase_name) + ' 2D phase arrival at ' + str(tt+tref) + ' secs' + ' +/- ' + str(dtt)  + ' secs ' + '\n'
                            solution_file.write(file_str)
                            file_str = str(ELAT[k]) + ' ' + str(ELON[k]) + ' ' +  str(EDEPTH[k]) + ' ' + str(SLAT[k]) + ' ' + str(SLON[k]) + ' ' + str(phase_name) + ' ' + str(tt+tref) + ' ' + str(dtt) + '\n'
                            solution_file1.write(file_str)

                            big_string = big_string + 'Phase name: '+ phase_name + '\n' + '2D travel time in seconds: ' + str(tt+tref)+ '\n' + 'Error +/- in seconds: ' + str(dtt)+ '\n' + '----------------------------------------------------------------------------'+ '\n' 
...
################################################################
    ...
    def on_plot_spec_released(self):
        ...
        if bool(self.ui.SEISGLOB2.checkState()):
            ...
            sp = SeisTomoPy.spectrum(model_spec,para,dep_spec,NSmax_spec)
        ...
        if bool(self.ui.YOURMODEL.checkState()):
            model_spec = 'MYMODEL'
            sp = SeisTomoPy.spectrum(model_spec,para,dep_spec,NSmax_spec)
            if bool(self.ui.norm.checkState()):
                sp4 =self.ax_spec.plot(sp[:,0], sp[:,1]/np.amax(sp[:,1]), linewidth=1.0,color="y",marker="d",markeredgecolor="k", label='MY MODEL')
            else:
                sp4 =self.ax_spec.plot(sp[:,0], sp[:,1], linewidth=1.0,color="y",marker="d", markeredgecolor="k",label='MY MODEL')
        if os.path.isfile(DIR + '/../fortran_files/input_files/map_file.xyz'):
        	sp = SeisTomoPy.spectrum_fromfile_GUI('../input_files/map_file.xyz',NSmax_spec)
    ...
    def on_saveoutput_spec_released(self):
        ...
        if os.path.isdir(str(selected_directory7) + '/output_spectre_' + \
            ...
            shutil.copytree(DIR + '/../fortran_files/output_files_spectre', str(selected_directory7) + '/output_spectre_' + \
             str(dep_spec) + 'km')
    ...
    def on_loadfile_spec_released(self):
        ...
        if self.spectre_file.endswith('.xyz'):
            ...
            filename6 = DIR + '/../fortran_files/input_files/map_file.xyz'
...
################################################################
    ...
    def on_plot_corr_released(self):
        ...
        if (model_corr1 != 'None') and (model_corr2 != 'None'):
            sp = SeisTomoPy.correlation(model_corr1,model_corr2,dep_corr1,dep_corr2,para1,para2,NSmax_corr)
            ...
            if os.path.isfile(DIR + '/../fortran_files/input_files/map_file1.xyz'):
                self.ui.label_33.setText('')
                sp = SeisTomoPy.correlation_fromfile_GUI(model_corr2,dep_corr2,para2,DIR + '/../fortran_files/input_files/map_file1.xyz',NSmax_corr)
                sp1 =self.ax_corr.plot(sp[:,0], sp[:,1], linewidth=1.0,color="grey",marker="d", label='from file-' +str(model_corr2))
            elif os.path.isfile(DIR + '/../fortran_files/input_files/map_file2.xyz'):
                self.ui.label_37.setText('')
                sp = SeisTomoPy.correlation_fromfile_GUI(model_corr1,dep_corr1,para1,DIR + '/../fortran_files/input_files/map_file2.xyz',NSmax_corr)
            ...
            if os.path.isfile(DIR + '/../fortran_files/input_files/map_file1.xyz') and os.path.isfile(DIR + '/../fortran_files/input_files/map_file2.xyz'):
                sp = SeisTomoPy.correlation_fromfile2_GUI(DIR + '/../fortran_files/input_files/map_file1.xyz',DIR + '/../fortran_files/input_files/map_file2.xyz',NSmax_corr)
                self.ui.label_33.setText('')
                self.ui.label_37.setText('')
    ...
    def on_saveoutput_corr_released(self):
        ...
        if os.path.isdir(str(selected_directory8) + '/output_corr'):
        	...
        	shutil.copytree(DIR + '/../fortran_files/output_files_corr', str(selected_directory8) + '/output_corr')
