...
c
c Corrected by S. Durand March 2017 for removing warnings
c       
c*************************************************************************
...
c
       ...
       do 1 i=1,nr
          ...
          umdt1=1.-dt1
c
                 data(i,j)=(provi(it,idint(ip))*umdt1+
     &                      provi(idint(itp1),idint(ip))*dt1)*umdt2+
     &                     (provi(it,idint(ipp1))*umdt1+
     &                      provi(idint(itp1),idint(ipp1))*dt1)*dt2
c
 1     continue
                   endif
c
...
5         itp1=k
          it=k-1
             dt1=(t-teta(it))/(teta(idint(itp1))-teta(it))
             umdt1=1-dt1
       do 10 j=1,nnd
          p=(j-1.)/c1+1.
          ip=p
...
c
               donnee(i,j)=(provi(it,idint(ip))*umdt1+
     &                      provi(idint(itp1),idint(ip))*dt1)*umdt2+
     &                     (provi(it,idint(ipp1))*umdt1+
     &                      provi(idint(itp1),idint(ipp1))*dt1)*dt2
 10    continue
