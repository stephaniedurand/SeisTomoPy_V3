...
ccccccccc1ccccccccc2ccccccccc3ccccccccc4ccccccccc5ccccccccc6ccccccccc7cc
       ...
       open(111,file="../log/cross.log")
       write(111,*) "START CORSS SECTION"
...
!       Normalisation as minos: 
       ...
       homedir=trim(adjustl(tampon))//'/SeisTomoPy_files/'
        ...
        write(111,*) "Read center lat, center lon and az"
...
!----------------------------------------------------------------------
       write(111,*) "Read pline functions"
       write(111,*) "Read model coefficients"
!----------------------------------------------------------------------
...
!----------------------------------------------------------------------
       write(111,*) "Compute the Great-Circle-Path" 
!----------------------------------------------------------------------
       ...
       open(6,file=trim(adjustl(homedir))//'output_files_cross/'//
     +'hotspots_profile.xy')
       open(5,file='../data/hotspots.xy')
       open(7,file='../data/catalogue.xy')  
       open(8,file=trim(adjustl(homedir))//'output_files_cross/'//
     +'eq_profile.xy')       
       write(6,*) "NaN ", "NaN"
       write(8,*) "NaN ", "NaN"
       pnts(1,1)=elon
...
!----------------------------------------------------------------------
       write(111,*) "Make profile .xyz file"
!----------------------------------------------------------------------
...
!      Comme dans S40RTS
     ...
     +       trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_SEISGLOB2_VP.res',
     +   trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_SEISGLOB2_VS.res',
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_SEISGLOB2_RHO.res',
     + pnts,NN,model,dep,width)
       endif

       if (tomo.eq.2) then
       call sph2geo2S40RTS(NSmax,p,np,radial_basis,
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_S40RTS_VP.res',
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_S40RTS_VS.res',
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_S40RTS_RHO.res',
     + pnts,NN,model,
     ...
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_S362WMANIM_VP.res',
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_S362WMANIM_VS.res',
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_S362WMANIM_RHO.res',
     + pnts,NN,model,
     ...
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_SEMUCBWM1_VP.res',
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_SEMUCBWM1_VS.res',
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_SEMUCBWM1_RHO.res',
     + pnts,NN,model,
     ...
     +       trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_SEISGLOB1_VP.res',
     +   trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_SEISGLOB1_VS.res',
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_SEISGLOB1_RHO.res',
     + pnts,NN,model,dep,width)
       endif
     ...
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_SP12RTS_VP.res',
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_SP12RTS_VS.res',
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_SP12RTS_RHO.res',
     + pnts,NN,model,
     ...
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_SGLOBE_VP.res',
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_SGLOBE_VS.res',
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_SGLOBE_RHO.res',
     + pnts,NN,model,
     ...
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_3D2016_VP.res',
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_3D2016_VS.res',
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_3D2016_RHO.res',
     + pnts,NN,model,
     +   dvvst7,579,dep,width)
       endif

       if (tomo.eq.9) then
       open(9,file=trim(adjustl(homedir))//'models/YOURMODEL_5km.sph')   
       cd=1
       do depth=2890,0,-5
     ...
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_MYMODEL_VP.res',
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_MYMODEL_VS.res',
     + trim(adjustl(homedir))//'output_files_cross/'//
     +'profile_MYMODEL_RHO.res',
     + pnts,NN,model,
     +   dvvst8,579,dep,width)
       endif

!----------------------------------------------------------------------
       write(111,*) "Files for plots"
!----------------------------------------------------------------------
      
       open(1,file=trim(adjustl(homedir))//'output_files_cross/'//
     +'dots_profile.xy')
